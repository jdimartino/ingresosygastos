const fs = require('fs');

const manualText = fs.readFileSync('manual_calc.txt', 'utf8');
const manualLines = manualText.split('\n').filter(l => l.trim() !== '');

let manualSum = 0;
const manualItems = [];

manualLines.forEach(line => {
    // Format: 2/2/2026 \t Comida (Mercado) \t 4
    const parts = line.split('\t');
    if (parts.length >= 3) {
        const monto = parseFloat(parts[2].trim());
        manualSum += monto;
        manualItems.push({ fecha: parts[0].trim(), categoria: parts[1].trim(), monto });
    }
});

console.log("=== CALCULO MANUAL ===");
console.log("Total Manual:", manualSum);
console.log("Cantidad de items manuales:", manualItems.length);

const dbText = fs.readFileSync('.gemini/antigravity/brain/b0c45fe6-b451-4bda-85c1-119f2d15a14c/.system_generated/steps/952/output.txt', 'utf8');
const dbLines = dbText.split('\n');

let dbSum = 0;
const dbItems = [];
let currentItem = {};

for (let line of dbLines) {
    line = line.trim();
    if (line.startsWith('- __path__:')) {
        if (Object.keys(currentItem).length > 0) {
            dbItems.push(currentItem);
        }
        currentItem = {};
    } else if (line.startsWith('fecha:')) {
        currentItem.fecha = line.split("'")[1]; // extract '2026-02-23'
    } else if (line.startsWith('categoria:')) {
        currentItem.categoria = line.substring(line.indexOf(':') + 1).trim();
    } else if (line.startsWith('monto:')) {
        currentItem.monto = parseFloat(line.split(':')[1].trim());
        dbSum += currentItem.monto;
    }
}
if (Object.keys(currentItem).length > 0) {
    dbItems.push(currentItem);
}

console.log("\n=== BASE DE DATOS ===");
console.log("Total DB:", dbSum);
console.log("Cantidad de items DB:", dbItems.length);

console.log("\n=== DIFERENCIAS ===");
// Simplistic comparison
const manualItemsSig = manualItems.map(i => `${i.categoria} - ${i.monto}`);
const dbItemsSig = dbItems.map(i => `${i.categoria} - ${i.monto}`);

const missingInDB = manualItemsSig.filter(x => {
    const idx = dbItemsSig.indexOf(x);
    if (idx > -1) {
        dbItemsSig.splice(idx, 1);
        return false;
    }
    return true;
});

console.log("Items en tu Excel pero NO en la base de datos:");
console.log(missingInDB);

console.log("\nItems en la base de datos pero NO en tu Excel:");
console.log(dbItemsSig);
